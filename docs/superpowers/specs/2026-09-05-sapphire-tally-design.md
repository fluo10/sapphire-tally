...
- 日付: 2026-09-05
- 状態: 承認待ち
- 関係プロジェクト: grain-id（ID方式）、sapphire-journal / sapphire-ledger（思想・構成の前例）
...
- `timestamp`: 任意。**フルISO8601（タイムゾーン付き）をフィールドとして持つ**。
...
- 不正なTOML・孤立スタンプ（存在しないactivity参照）はスキャン時に**スキップして警告ログ**。
  一部ファイルの破損で全体が読めなくなるのを防ぐ
- grain-id衝突は事実上起きないが、起きたら上書きせずエラー
